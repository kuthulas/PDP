...
enum cache_policy {
  ...
  PDP     /* replace unprotected block in the set */
};
...
{
  md_addr_t address;
  struct pdp_fifo_node *next;
  struct pdp_fifo_node *prev;
};
...
struct cache_blk_t
{
  ...
  int rpd;    /* remaining protecting distance */
  int reused; /* reuse bit; 0 - inserted line, 1 - reused line */
};
...
struct cache_set_t
{
  ...
  struct pdp_fifo_node *fifo_head;  /* Per-set FIFO with block addresses */
  struct pdp_fifo_node *fifo_tail;
  int nfifo; /* current number of entries in FIFO */
};
